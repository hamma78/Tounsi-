...
const TermsOfServiceScreen: React.FC<TermsOfServiceScreenProps> = ({ navigate }) => {
  const { t, language } = useLanguage();
  const termsPoints = Array.from({ length: 10 }, (_, i) => `termsPoint${i + 1}`);
  ...
  return (
    <div className="max-w-4xl mx-auto text-white p-4 md:p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-bold">{t('termsTitle')}</h2>
      </div>
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 space-y-4">
        <p className="text-gray-300">{t('termsIntro')}</p>
        <ul className="list-disc list-inside space-y-3 text-gray-400">
          {termsPoints.map(pointKey => (
